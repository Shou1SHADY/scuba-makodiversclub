...
import { Plus, Edit2, Trash2, Loader2, BookOpen, X, PlusCircle, Upload, Eye, Check, Calendar } from "lucide-react";
...
export default function CoursesAdminPage() {
    ...
    const emptyForm = {
        ...
        duration: "",
        highlights: [""],
        itinerary: [{ day: "Module 1", activities: [""] }],
        included: [""],
        notIncluded: [""],
        images: [] as string[],
    ...
    const handleOpenDialog = (course?: any) => {
        if (course) {
            ...
            const c = {
                ...
                itinerary: course.itinerary?.length > 0 ? course.itinerary : [{ day: "Module 1", activities: [""] }],
                included: course.included?.length > 0 ? course.included : [""],
                notIncluded: (course.notIncluded || course.not_included)?.length > 0 ? (course.notIncluded || course.not_included) : [""],
                images: course.images || [],
                customDetails: course.customDetails || [],
            };
            setFormData(c);
        } else {
    ...
    const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (const file of Array.from(files)) {
            const fileExt = file.name.split(".").pop();
            const filePath = `trip-images/course_${Date.now()}_${Math.random().toString(36).substring(2, 8)}.${fileExt}`;
            const { error } = await supabase.storage.from("trip-images").upload(filePath, file, { upsert: true });
    ...
    // Generic array helpers (included / notIncluded)
    ...
    // Itinerary helpers
    const addItineraryDay = () => setFormData({ ...formData, itinerary: [...formData.itinerary, { day: `Module ${formData.itinerary.length + 1}`, activities: [""] }] });
    ...
    const removeItineraryDay = (dayIndex: number) => {
        ...
        setFormData({ ...formData, itinerary: itin.length > 0 ? itin : [{ day: "Module 1", activities: [""] }] });
    };
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col">
                        <div className="p-10 space-y-10 max-h-[55vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <Textarea
                                        ...
                                        placeholder="A longer description shown on the course detail page..."
                                        rows={4}
                                    />
                                </div>
                                <div className="space-y-3">
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-sm font-black uppercase tracking-[0.3em] text-emerald-400 flex items-center gap-3">
                                        <div className="h-px flex-1 bg-white/10" /> COURSE STRUCTURE <div className="h-px flex-1 bg-white/10" />
                                    </h3>
                                ...
                                <div className="space-y-3">
                                    {(formData.included || []).map((item: string, index: number) => (
                                        <div key={index} className="flex items-center gap-3">
                                            ...
                                            <Input value={item} onChange={e => updateArrayItem("included", index, e.target.value)} className="h-12 bg-white/5 border-white/5 rounded-xl px-4 text-white text-sm flex-1" placeholder="e.g. All training materials" />
                                ...
                                <div className="space-y-3">
                                    {(formData.notIncluded || []).map((item: string, index: number) => (
                                        <div key={index} className="flex items-center gap-3">
                                            ...
                                            <Input value={item} onChange={e => updateArrayItem("notIncluded", index, e.target.value)} className="h-12 bg-white/5 border-white/5 rounded-xl px-4 text-white text-sm flex-1" placeholder="e.g. Equipment rental" />
                                ...
                                <div className="space-y-6">
                                    {(formData.customDetails || []).map((section: CustomDetailSection, secIndex: number) => (
                                        <div key={secIndex} className="bg-white/[0.03] border border-white/5 rounded-2xl p-6 space-y-4">
                                            <div className="flex items-center gap-3">
                                                <Input value={section.header} onChange={e => updateCustomHeader(secIndex, e.target.value)} className="h-12 bg-white/5 border-white/10 rounded-xl px-4 text-white text-sm font-bold flex-1" placeholder="Section header (e.g. Prerequisites)" />
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-black uppercase tracking-[0.3em] text-primary flex items-center gap-3">
                                    <div className="h-px flex-1 bg-white/10" /> COVER IMAGE <div className="h-px flex-1 bg-white/10" />
                                </h3>
                                <div className="space-y-3">
