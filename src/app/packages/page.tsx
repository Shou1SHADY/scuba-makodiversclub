    ...
    return (
        <div className="bg-brand-navy min-h-screen">
            ...
            <div className="container-width px-4 md:px-6 pb-20 md:pb-32 relative z-10">
                {loading ? (
                    <div className="flex justify-center p-20 bg-brand-navy rounded-[2rem] border border-white/5 shadow-2xl">
                        <Loader2 className="w-12 h-12 animate-spin text-primary" />
                    </div>
                ) : packages.length === 0 ? (
                    <div className="flex justify-center p-20 bg-brand-navy rounded-[2rem] border border-white/5 shadow-2xl text-white/50 text-center text-lg italic">
                    ...
                    <div className="grid gap-10 md:gap-16">
                        {packages.map((pkg, i) => (
                            ...
                            <div className="lg:w-[65%] p-6 md:p-10 lg:p-14 flex flex-col justify-center bg-white/[0.02]">
                                ...
                                <div className="mb-10 p-5 rounded-2xl bg-primary/5 border border-primary/15">
                                    <div className="flex items-center gap-2 mb-3">
                                        <CalendarDays size={14} className="text-primary" />
                                        <span className="text-primary uppercase text-[10px] font-bold tracking-[0.25em]">2026 Departures</span>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {pkg.dates.map((date, idx) => (
                                            <span
                                                key={idx}
                                                className="inline-flex items-center gap-1.5 text-white/80 text-xs font-semibold bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg hover:border-primary/30 hover:bg-primary/10 transition-all"
                                            >
                                                {date}
                                            </span>
                                        ))}
